...
io.on("connection", socket => {
  socket.on("join", async ({ user }) => {
    try {
      ...
      let files = data.files.map((x, i) => {
        return { id: x, filename: data.filenames[i], query_guss: settings.gussTheQuery(data.filenames[i]) };
      });
...
router.post("/upload", async (req, res) => {
  try {
    ...
    await document(user, root + "/subtitles/" + oid + ".srt", {
