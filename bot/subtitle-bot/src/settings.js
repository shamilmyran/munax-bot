...
const { bot, adminId } = require("../../config"); // 👈 unified config
...
const writeFile = async (path, data, json) => {
  try {
...
const settings = {
  to_inline_keyboard: (data, page, cb) => {
    let row = [];
    var eTxt = "Not Specified";
    for (var i in data) {
      row.push([{
        text: `[${data[i].Type}] ` + data[i].Title,
    ...
    row.push([{
      text: "<==",
      callback_data: JSON.stringify({ type: "back", page })
    }, {
      text: "==>",
      callback_data: JSON.stringify({ type: "next", page })
    }], [{
      text: "PAGE : " + page,
      callback_data: JSON.stringify({ type: "page_no" })
    }]);
    cb(row);
  },
  addUser: (query) => {
  ...
  isAvailbleLang: (langName) => {
    ...
    for (var i in allLangs) {
      if (allLangs[i].name.toLowerCase() == langName ||
          allLangs[i].local.toLowerCase() == langName ||
          allLangs[i]["1"].toLowerCase() == langName ||
          allLangs[i]["2"].toLowerCase() == langName ||
          allLangs[i]["3"].toLowerCase() == langName) {
        lang = allLangs[i];
        break;
      }
    }
  ...
  to_inline_keyboard_subs: (subs, userData, page) => {
    ...
    var nextPrevBtn = [{
      ...
      text: "==>",
      callback_data: JSON.stringify({ type: "next_page_sub", page })
    }];
    var userDataBtn = [{
      text: userData.title,
      callback_data: JSON.stringify({
  ...
  gussTheQuery: (q) => {
    ...
    query = {
      title: query[1],
      year: (y ? y[2].replace(/\(|\)/g, "") : null),
      episode: (e ? e[4] : null),
      season: (s ? s[3] : null),
      quality: (quality ? quality[2] : null),
      nonsearch: (nonsearch ? nonsearch.slice(2, nonsearch.length).join("") : null)
    };
    return query;
  },
  ...
  toMarkupBtnSSSubs: (sub, page, lang) => {
    ...
    let avl_l = true;
    if (!settings.isAvailbleLang(lang)) avl_l = false;
    btn.push([["<==", { t: "csss", ct: "p", tp, p: page, l: lang }], ["==>", { t: "csss", ct: "n", tp, p: page + 2, l: lang }]], [[`(${page + 1}/${tp})`, { t: "tpss" }]], [(avl_l ? ["Search in opensubtitles", { t: "slos", l: settings.isAvailbleLang(lang)["2"] }] : ["Search in opensubtitles", { t: "ss", w: "os" }])]);
    return btn;
  },
  addGroup: async chat => {
    console.log(chat);
  ...
  markupSDSearch: (subs, page) => {
    ...
    btns.push([["<--", { t: "csrsd", p: page, tp, ct: "p" }], ["-->", { t: "csrsd", p: page, tp, ct: "n" }]], [[`Page (${page}/${tp})`, { t: "srdsd" }]], [["🔍 SUBSCENE", { t: "ss", w: "ss" }], ["🔍 OPENSUBTITLES", { t: "ss", w: "os" }]]);
    return btns;
  },
  markupSDSeason: (res) => {
    let btns = [];
