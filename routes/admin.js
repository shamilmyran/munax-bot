...
const bitly = new bitlyClient(process.env.BITLY_TOKEN || "ae0cda66640cd89148dd99c9e90018ddcbe04808"),
db=require("../helper/db"),
ExpressBrute = require('express-brute'),
MongoStore = require('express-brute-mongo-update');

// Determine base URL – works on Render and Heroku
let baseUrl = process.env.RENDER_EXTERNAL_URL || process.env.HEROKU_URL || "http://localhost:3000"
let web = baseUrl.split("//")[1].split(".")[0]
// You can customize this logic or just set a flag
let isMainSite = (web === "munax" || web === "localhost") // adjust as needed
...
const loginCheck = (req, res, next)=> {
  try{
    if ((req.session.admin && req.session.admin.login) || 
        (req.query.own && req.query.own === "owntgwayurl@")) { // keep the old bypass if needed
      next()
    } else {
...
router.post("/login/update", bruteforce.prevent, (req, res)=> {
  var data = req.body
  // Use environment variables for credentials, fallback to defaults (change these!)
  const adminUser = process.env.ADMIN_USERNAME || "munax"
  const adminPass = process.env.ADMIN_PASSWORD || "munax@786" // change this!
...
router.get("/bitly", async(req, res)=> {
  ...
  bitly.shorten(url).then(r=> {
    res.json({ err: false, link: r.link })
  }).catch(err=> {
    res.json({ err: true })
  })
})
...
router.get("/getAds", loginCheck, async(req, res)=>{
  ...
  if(!sts) return res.end("unexpected error occurred while getting ads details on database")
  res.render("admin/addAds/ads", { title: "Available Ads Page", ads, org: web })
})

router.get("/vip", loginCheck, async(req, res)=>{
  try{
    res.render("admin/vip/main", { title: "VIP User Management", org: web })
  } catch(e){
    console.log(e);
    return
  }
...
router.get("/getUser", loginCheck, async(req, res)=>{
  try{
    if(req.query.id){
      var url = `${process.env.RENDER_EXTERNAL_URL || process.env.HEROKU_URL || "http://localhost:3000"}/api/translate/bot`
      console.log(url);
...
router.get("/vip/getData", async(req, res)=>{
  try{
    ...
    { body } = await request({ url: `${process.env.RENDER_EXTERNAL_URL || process.env.HEROKU_URL || "http://localhost:3000"}/admin/getUser?id=${req.query.id}&own=owntgwayurl@`, method: "get" });
    if(!user) throw "This User Is Not In VIP List"
    body = JSON.parse(body)
    if(!body.sts) throw body.err
    res.json({ sts: true, data: user, user: body.data })
  } catch(e){
    if(e instanceof Error || typeof e === "object") e = e.toString()
